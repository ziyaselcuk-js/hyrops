...
module.exports.run = async (client, message, args) => {
 ...
 if (!message.member.hasPermission("BAN_MEMBERS")) return message.channel.send(":no_entry: You must have the 'Ban Members' authority to use this command.");
    if (!args[0]) {
        return message.channel.send(`:x: Hey You Need To Specify A User's ID To Use This Command!`)
    }
    var sebeb = args.slice(1).join(" ");
    var seyfooo = args[0]
    var now = new Date()
    if (!sebeb) {
        message.guild.fetchBans()
            .then(bans => {
                if (bans.has(seyfooo)) {
                    return message.channel.send(`:x: This User Is Already Banned.`)
                }
                message.guild.ban(seyfooo, sebeb)
                    .then(async (member) => {
                        let user;
                        ...
                        message.channel.send(`<@!${user.id}> Banned `);
                    })
                    .catch(error => {
