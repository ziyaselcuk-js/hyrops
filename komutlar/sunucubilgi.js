...
const embed = new Discord.MessageEmbed()
 ...
 .addField(`Kanallar [${message.guild.channels.cache.size}]`, `${message.guild.channels.filter(c => c.type === "text").size} Yazı | ${message.guild.channels.filter(c => c.type === "voice").size} Ses | ${message.guild.channels.filter(c => c.type === 'category').size} Kategori`)
 .addField(`Kullanıcılar [${message.guild.members.cache.size}]`, `${message.guild.members.cache.filter(m => m.user.presence.activities === "online").size} Çevrimiçi | ${message.guild.members.cache.filter(m => m.user.presence.activities === "offline").size} Çevrimdışı | ${message.guild.members.cache.filter(m => m.user.presence.activities === "dnd").size} Rahatsız Etmeyin | ${message.guild.members.cache.filter(m => m.user.presence.activities === "idle").size} Boşta | ${message.guild.members.cache.filter(m => m.user.bot).size} Bot`)
...
exports.conf = {
	enabled: true,
	guildOnly: true,
	aliases: ['sunucu-bilgi', 'sunucubilgi', 'server-info'],
	permLevel: 0,
	kategori: 'kullanıcı'
}
