...
.addField('Kullanım', `h!bilgi emojiadi
h!bilgi :emoji:`)
.addField('Örnekler', `h!bilgi ${client.emojis.cache.get(client.emojis.cache.random().id)}
h!bilgi superemoji`)
.addField('Kısaltmalar', `h!bılgı
h!bilgı
h!bılgi`)
.setFooter(message.author.username+' tarafından istendi.', message.author.avatarURL({ dynamic: true })));
  ...
  var emoji;
  if(s) {
  if(!message.guild.emojis.cache.get(s.split(':')[2].split('>')[0])) return message.channel.send('Bilgisini vermem için **bu sunucuda** olan bir emojiyi göndermeli veya adını yazmalısınız. Örnek; `h!bilgi emoji`')
  emoji = message.guild.emojis.cache.get(s.split(':')[2].split('>')[0]);
  } else {
  if(!message.guild.emojis.cache.find(x => x.name === args.slice(0).join('-'))) return message.channel.send('Bilgisini vermem için **bu sunucuda** olan bir emojiyi göndermeli veya adını yazmalısınız. Örnek; `h!bilgi emoji`')
...
exports.conf = {
  enabled: true,
  guildOnly: false,
  aliases: ['bılgı', 'bilgı', 'bılgi', 'info', 'ınfo'],
  permLevel: 0
};
